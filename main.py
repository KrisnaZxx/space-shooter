...
# Warna (dipakai untuk HUD, peluru, efek)
BLACK  = (0,   0,   0)
WHITE  = (255, 255, 255)
YELLOW = (255, 255, 0)
RED    = (220, 50,  50)
...
# =============================================
# ASSET LOADER
# Semua gambar dimuat sekali di sini supaya
# tidak dimuat ulang setiap frame (efisien).
# =============================================
ASSET_DIR = os.path.join(os.path.dirname(__file__), "assets")

def load_sprite(filename, crop, scale=None):
    """
    Memuat satu sprite dari sprite sheet.
    crop  = (x, y, w, h) — posisi & ukuran di dalam sheet.
    scale = (w, h)        — ukuran akhir di layar (opsional).
    """
...
def load_background(filename, target_w, target_h):
    """Memuat background dan scale ke ukuran layar."""
    path = os.path.join(ASSET_DIR, filename)
...
# --- Muat semua asset ---
try:
    # ---- Player sprites (SpaceShips_Player-0001.png, 256x256) ----
    # Baris 0: 4 fighter kecil (abu, hijau, biru, orange)
    #     # crop = (x, y, lebar, tinggi) di dalam sprite sheet
    IMG_PLAYER = [
        load_sprite("SpaceShips_Player-0001.png", (12, 22, 39, 41), (58, 62)),  # abu
        load_sprite("SpaceShips_Player-0001.png", (76,  22, 39, 41), (58, 62)),  # hijau
        load_sprite("SpaceShips_Player-0001.png", (140, 22, 39, 41), (58, 62)),  # biru
        load_sprite("SpaceShips_Player-0001.png", (204, 22, 39, 41), (58, 62)),  # orange
    ]

    # ---- Enemy sprites (SpaceShips_Enemy-0001.png, 256x256) ----
    # 3 baris = 3 warna variant; kolom 0 = kapal besar, kolom 2 = kapal kecil
    IMG_ENEMY_BIG = [
        load_sprite("SpaceShips_Enemy-0001.png", (33, 10,  47, 54), (56, 64)),  # abu-merah
        load_sprite("SpaceShips_Enemy-0001.png", (33, 97,  47, 56), (56, 64)),  # orange
        load_sprite("SpaceShips_Enemy-0001.png", (33, 187, 47, 53), (56, 64)),  # ungu
    ]
    IMG_ENEMY_SMALL = [
        load_sprite("SpaceShips_Enemy-0001.png", (150, 10,  32, 54), (36, 52)),
        load_sprite("SpaceShips_Enemy-0001.png", (150, 97,  32, 56), (36, 52)),
        load_sprite("SpaceShips_Enemy-0001.png", (150, 187, 32, 53), (36, 52)),
    ]

    # ---- Asteroid sprites (Asteroids-0001.png, 128x128) ----
    # TL = merah besar, BL = abu besar, TR = merah kecil, BR = abu kecil
    IMG_ASTEROID = [
        load_sprite("Asteroids-0001.png", (6,  30, 50, 32), (52, 52)),   # merah besar
        load_sprite("Asteroids-0001.png", (6,  69, 39, 38), (52, 52)),   # abu besar
        load_sprite("Asteroids-0001.png", (83, 16, 29, 45), (34, 34)),   # merah kecil
        load_sprite("Asteroids-0001.png", (70, 69, 39, 38), (34, 34)),   # abu kecil
    ]

    # ---- Background (320x320 → 800x600) ----
    IMG_BG = load_background("Background_Full-0001.png", SCREEN_WIDTH, SCREEN_HEIGHT)

    ASSETS_LOADED = True
    print("[INFO] Asset berhasil dimuat!")

except Exception as e:
    print(f"[WARNING] Asset tidak ditemukan: {e}")
    print("[INFO] Game berjalan tanpa asset (mode shape).")
    ASSETS_LOADED   = False
    IMG_PLAYER      = None
    IMG_ENEMY_BIG   = None
    IMG_ENEMY_SMALL = None
    IMG_ASTEROID    = None
    IMG_BG          = None
...
class GameObject:
    """
    Parent class untuk semua objek dalam game.
    Menyimpan atribut dasar: posisi, ukuran, warna, dan status aktif.
    Menggunakan enkapsulasi (private attribute + getter/setter).
    """
    ...
    def get_rect(self):
        """Mengembalikan Rect untuk collision detection."""
        return pygame.Rect(self.__x, self.__y, self.__width, self.__height)

    def draw(self, surface):
        """Di-override oleh child class."""
        pass
    ...
    def __del__(self):
        """Destruktor — dipanggil saat objek dihapus dari memori."""
        pass


# =============================================
# CLASS PLAYER (inherits GameObject)
# =============================================
class Player(GameObject):
    """
    Pesawat player yang dikendalikan keyboard.
    Inheritance dari GameObject.
    """
    def __init__(self, x, y, skin=1):
        ...
        self.__shoot_delay    = 15        # frame antar tembakan
        self.__invincible     = 0         # frame kebal setelah kena hit
        self.__bullets        = []
        self.__skin           = skin      # indeks warna pesawat (0–3)
    ...
    def shoot(self):
        """Menembakkan peluru dari tengah pesawat jika cooldown habis."""
        if self.__shoot_cooldown <= 0:
            cx = self.get_x() + self.get_width() // 2 - 3
            self.__bullets.append(Bullet(cx, self.get_y(), speed=11,
                                         color=CYAN, direction=-1))
            self.__shoot_cooldown = self.__shoot_delay

    def take_damage(self, amount):
        """Menerima damage; dilindungi invincible frames agar tidak double-hit."""
...
class Bullet(GameObject):
    """
    Peluru yang ditembakkan player atau musuh.
    Inheritance dari GameObject.
    """
    def __init__(self, x, y, speed=11, color=CYAN, direction=-1):
        super().__init__(x, y, 6, 16, color)
        self.__speed     = speed
        self.__direction = direction    # -1 ke atas (player), +1 ke bawah (musuh)
...
# =============================================
# CLASS ENEMY — BASE MUSUH (inherits GameObject)
# =============================================
class Enemy(GameObject):
    """
    Base class untuk semua jenis musuh.
    Inheritance dari GameObject.
    Child class: Asteroid, AlienShip.
    """
    ...
    def _add_bullet(self, bullet):
        """Dipakai child class untuk menambah peluru."""
    ...
    def draw(self, surface):
        pass   # di-override child class


# =============================================
# CLASS ASTEROID (inherits Enemy)
# =============================================
class Asteroid(Enemy):
    """
    Musuh asteroid — bergerak lurus ke bawah dan berputar.
    Inheritance dari Enemy → GameObject.
    """
    def __init__(self, x, y):
        self.__variant = random.randint(0, 1)    # 0=merah, 1=abu
        size  = 52
        speed = random.uniform(1.5, 3.2)
        super().__init__(x, y, size, size, GRAY, health=2, speed=speed)
...
# =============================================
# CLASS ALIENSHIP (inherits Enemy)
# =============================================
class AlienShip(Enemy):
    """
    Musuh kapal alien — zigzag dan bisa menembak ke bawah.
    Inheritance dari Enemy → GameObject.
    """
    def __init__(self, x, y):
...
# =============================================
# CLASS POWERUP (inherits GameObject)
# =============================================
class PowerUp(GameObject):
    """
    Item yang bisa dipungut player untuk bonus.
    Inheritance dari GameObject.
    """
...
# =============================================
# CLASS STAR — Bintang background
# =============================================
class Star(GameObject):
    """Bintang animasi latar belakang (dipakai jika asset BG tidak ada)."""
    def __init__(self):
...
# =============================================
class Game:
    """Mengatur semua state game: menu, countdown, playing, game_over."""

    def __init__(self):
        self.__player          = Player(SCREEN_WIDTH // 2 - 29,
                                        SCREEN_HEIGHT - 110, skin=3)
        self.__enemies         = []
        self.__powerups        = []
        ...
        self.__bg_y            = 0       # posisi scroll background
    ...
    def draw_hud(self):
        ...
        screen.blit(wave_surf,
                    (SCREEN_WIDTH // 2 - wave_surf.get_width() // 2, 10))
    ...
    def draw_menu(self):
        ...
        # Tampilkan pesawat player di menu
        if ASSETS_LOADED and IMG_PLAYER:
    ...
    def run(self):
        ...
        while running:
            ...
            # Bintang selalu update (fallback tanpa asset)
